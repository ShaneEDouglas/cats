...
def pick(paragraphs, select, k):
  ...
  for paragrapgh in paragraphs:
      if select(paragrapgh):
          store_paragraphs.append(paragrapgh)

  if k < len(store_paragraphs):
      return store_paragraphs[k]
  else:
      return ''
...
def about(subject):
  ...
  # BEGIN PROBLEM 2
  def select(paragrapgh):
    clean_paragrpah = remove_punctuation(lower(paragrapgh))
    words = split(clean_paragrpah)
...
def accuracy(typed, source):
  ...
  for typed_word, source_word in zip(typed_words, source_words):
    if typed_word == source_word:
      correct_words += 1
  accuracy = correct_words / len(typed_words) * 100
  return accuracy
  # END PROBLEM 3
...
def wpm(typed, elapsed):
  ...
  # BEGIN PROBLEM 4
  typed_words = len(typed) / 5


  time_mintues = elapsed / 60

  # calculate wpm
  words_per_minute = typed_words / time_mintues
...
def autocorrect(typed_word, word_list, diff_function, limit):
  ...
  min_difference = diff_function(typed_word, best_match, limit)
...
def feline_fixes(typed, source, limit):
  """As diff function for autocorrect that determines how many letters
  in TYPED need to be substituted to create SOURCE, then adds the difference in
  their lengths and returns the reult.
...
def minimum_mewtations(typed, source, limit):
  ...
  if not typed or not source: # Feel free to remove or add additional cases
      ...
      add = 1 + minimum_mewtations(typed, source[1:], lmit -1) # Fill in these lines
      remove = 1 + minimum_mewtations(typed[1:], source, limit -1)
      substitute = 1 + minimum_mewtations(typed[1:], source[1:], lmit -1)
...
def report_progress(typed, source, user_id, upload):
  ...
  correct = 0
  for type_word, source_word in zip(typed,source):
      if type_word == source_word:
          correct += 1
      else:
          break
...
def time_per_word(words, timestamps_per_player):
  ...
  for player_timestamps in timestamps_per_player:
      player_times = [] # will hold the indivual times for each word for one player
      for i in range(1, len(player_timestamps)):
          difference  = player_time[i] - player_time[i - 1]
          player_times.append(difference)
...
def fastest_words(match):
  ...
  for word_inidex in word_indices:
      fastest_time = float('inf')
      fastest_player_index = None

      for player_index in player_indices:
          current_time = time(match, player_index, word_inidex)

          if current_time < fastest_time:
              fastest_time = current_time
              fastest_player_index = player_index
      fastes_word = get_word(match, word_index)
      result[fastest_player_index].append(fastes_word)
